...
//	m_slayer_ispk_file.open(ISPK_SLAYER, std::ios::binary|ios::nocreate);
...
//	m_vampire_ispk_file.open(ISPK_VAMPIRE, std::ios::binary|ios::nocreate);
...
//	}
	...
	/*
	ifstream file_ac_slayer( CFPK_ADVANCEMENT_SLAYER, std::ios::binary | ios::nocreate );
	if( !file_ac_slayer )
		_Error( FILE_OPEN );
	m_AdvancementSlayerCfpk.LoadFromFile( file_ac_slayer );
	file_ac_slayer.close();
	
	ifstream file_ac_vampire( CFPK_ADVANCEMENT_VAMPIRE, std::ios::binary | ios::nocreate );
	if( !file_ac_vampire )
		_Error( FILE_OPEN );
...
//			wsprintf(sz_filename, "UserSet\\%s.set", g_char_slot_ingame.sz_name);
//			
//			ifstream file(sz_filename, std::ios::binary | ios::nocreate);
//			
//			if(file)
//			{
