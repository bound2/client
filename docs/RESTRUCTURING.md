...
- [x] **5.4 Format-string audit** (code-health C19/C20/C22, the sprintf
  ...
  >
  > **Fourth slice (2026-09-04): the last 37, and `R7 37 → 0`.** No call
  > site in this client hands a `Data/Info/String.inf` entry to a printf
  > as its format any more, so the ratchet holds a line from here rather
  > than tracking a retreat: a new one fails the suite. Finding C19 is
  > closed — see its entry in the code-health review for what "closed"
  > does and does not mean, because it does not mean a corrupt table
  > prints correctly.
  >
