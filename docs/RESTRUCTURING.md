...
- [ ] **5.4 Format-string audit** (code-health C19/C20/C22, the sprintf
  ...
  > **35 unit tests / 56 checks** in `tests/unit/test_safe_format.cpp`,
  ...
  > `GCBloodBibleListHandler` indexed the table with a packet field —
  > and `CTypeTable::operator[]` only bounds-checks under `_DEBUG`, so
  > outside Debug a long enough list read past the end of the table;
  > those arguments go through `GetGameString()`, which range-checks in
  > every build. And four sites in `GCPartyLeaveHandler` put the
  ...
  > `VS_UI_GameCommon.cpp` alone holds about 60.
