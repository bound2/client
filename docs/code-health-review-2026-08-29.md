...
By severity: **Critical 27 fixed / 1 open** (C19, narrowed — see its entry), **High 38 fixed, 3 noted, 19 open** (36/21 until the two build findings closed on 2026-09-02), **Medium 13 fixed / 68 open**, **Low 3 fixed / 25 open**.
...
>
> **Second pass, 2026-09-03** (branch `harden/checked-format`, `docs/RESTRUCTURING.md` task 5.4). The argument-passing half now has a fix rather than a gate, and a number rather than an estimate. The remaining population is **288 call sites**, counted by ratchet **R7**; "~140" and the "roughly 600" in this finding's title were both estimates, and the second was of a different thing (all `sprintf` in the client).
>
...
>
> **224 of the 288 are converted** — every `sprintf`/`wsprintf` in `Client/PacketHandler` fed by the table. `GCBloodBibleListHandler` also `sprintf`ed a `char[192]` plus a `"%3d "` prefix into another `char[192]`, four bytes short, which the bounded formatter closes. Its table lookups are indexed by a packet field and go through `GetGameString()` now — not because the index was unchecked, as the first draft of this note claimed, but because an out-of-range `CTypeTable` lookup yields a default-constructed `MString` whose `GetString()` is NULL, and a NULL reaching a `%s` is undefined rather than empty. The second slice took all 193 `VS_UI` sites the same way, so what is left is the executable's own 64 - three `AddFormat` calls in that tree and 61 elsewhere in `Client`. Both slices are audited by `tests/tools/check_format_arity.pl`, a ctest that compares every converted site's arguments against the built-in English table and fails when an entry asks for more than the call site passes: 219 of the 224 sites are checkable there, with no failures. Neither half of the defence can tell that an entry *means* what its call site passes — a mistranslated entry still prints wrongly. What is gone, at a converted site, is printing something the caller never owned.
