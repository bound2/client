...
By severity: **Critical 28 fixed / 0 open** (C19 closed 2026-09-04; its entry says what "closed" does and does not mean), **High 38 fixed, 3 noted, 19 open** (36/21 until the two build findings closed on 2026-09-02), **Medium 13 fixed / 68 open**, **Low 3 fixed / 25 open**.

**Every finding the review rated Critical is now closed** — C19, the last of them, on 2026-09-04. That does not mean the client is free of critical defects: the pointer-truncation pass found a whole family of the same class that the review never recorded, described under *Open, not in this review* below.
...
| Data-file strings as printf formats: all 293 call sites checked at the point of use (C19) | 🔴 Critical | `31f5f2f`, then `docs/RESTRUCTURING.md` task 5.4 |
...
> ✅ **Fixed**, 2026-09-04, by `docs/RESTRUCTURING.md` task 5.4 across four slices. **Ratchet R7 is 0**: no call site in this client hands a `String.inf` entry to a printf-family function as its format any more. All 293 go through `SafeFormat`, which checks the entry's conversions against the arguments the call site actually passed — a conversion with no argument left, an argument of the wrong type, a `%n`, or an argument-supplied width is printed as text rather than performed, and every destination is bounded by its own size. The finding's failure scenario — a tampered entry causing an arbitrary write through `%n`, or a wild `char*` read through a surplus `%s` — is closed at every site, and R7 holds the line at zero rather than tracking a retreat.
>
> **What "closed" does not mean.** A corrupted or hostile `String.inf` can still produce *wrong text*: entries are data, and nothing can know that an entry means what its call site passes. What it can no longer do is make the program read or write something it was never given. Two limits are stated rather than papered over. `tests/tools/check_format_arity.pl` audits arity against the built-in English table — which is what the default build formats with, since `InitGameStringTable` installs it over the file data — but no `String.inf` ships in this repository, so the `LANGUAGE != 3` entries cannot be checked here at all; that path rests on the run-time refusal, which is where it matters most. And of 385 conversion positions only 117 get a *type* comparison, because an argument whose kind cannot be told is deliberately not a failure; the arity half, which is the half this finding turns on, covers all 285 resolvable sites.
>
...
>
> **All 293 are converted** — every `sprintf`/`wsprintf` in `Client/PacketHandler` fed by the table. `GCBloodBibleListHandler` also `sprintf`ed a `char[192]` plus a `"%3d "` prefix into another `char[192]`, four bytes short, which the bounded formatter closes. Its table lookups are indexed by a packet field and go through `GetGameString()` now — not because the index was unchecked, as the first draft of this note claimed, but because an out-of-range `CTypeTable` lookup yields a default-constructed `MString` whose `GetString()` is NULL, and a NULL reaching a `%s` is undefined rather than empty. The second slice took all 198 `VS_UI` sites the same way, so a third slice took the 27 `AddFormat` sites through a new `CMessageArray::AddSafeFormat`, which packs its arguments with their types rather than as varargs. What is left is **37** ordinary `sprintf` sites, all executable-side: `UIMessageManager.cpp` 14, `MTopView.cpp` 10, `GameUI.cpp` 7, `ModifyStatusManager.cpp` 3, `CGameUpdate.cpp` 2, `PacketFunction.cpp` 1. All three slices are audited by `tests/tools/check_format_arity.pl`, a ctest that compares every converted site's arguments against the built-in English table and fails when an entry asks for more than the call site passes: 250 of the 256 sites are checkable there, with no failures. Neither half of the defence can tell that an entry *means* what its call site passes — a mistranslated entry still prints wrongly. What is gone, at a converted site, is printing something the caller never owned.
