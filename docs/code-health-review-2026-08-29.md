...
> ✅ **Fixed** in `76a1185` and reworked in `3bc340e` (branch `harden/network-input`). The blanket separator ban in the first attempt broke legitimate transfers -- the wire value is the sender's relative profile path -- so the validator now rejects drive colons, absolute paths, `..` components, Win32 reserved device names and trailing dots/spaces instead, and the reject path frees the transfer state before disconnecting. Confining transfers to the profile directory is a follow-up; a peer can still name a benign relative path under the working directory.
