...
The test-driven work is confined to code compiled into a static library, since that is the only code a test binary can link against. A second phase has since fixed ten defects in the `DarkEden` executable itself, listed under Runtime defects below; those were found by running the client rather than by this review, and none of them are among the 197 findings.
...
| Six rows of `C_VS_UI_ASK_DIALOG::m_sz_question_msg` never assigned, and read as format strings | any of the six friend dialogs ran `strlen()` over an indeterminate pointer and handed it to `sprintf` as a format. Upstream commented the `ASK_FRIEND_*` assignments — and their string ids — out of `InitString()` while leaving the six cases that read them live; `m_sz_question_msg` is an ordinary member array, so nothing else gave those rows a value. Reachable from a `GCFriendChatting` packet, which makes it server-triggered rather than merely latent | task 5.4's fifth slice |
...
> 2. **R8 = 13** — the new ratchet, which asks the question R7 cannot: at a printf-family call, is the format a string literal? Only 13 calls in `Client`, `VS_UI` and `basic` take a format that is not, and all 13 were read: 11 vararg forwarders where the format is the function's own parameter, and 2 `sprintf(szTemp, TEXT("..."))` where a literal hides behind a macro.
> 3. **The 11 forwarders' own callers**, swept for a non-literal first argument: **zero**.
> 4. **The family list came from the tree, not from memory** — every identifier matching `*printf*` in `Client`, `VS_UI` and `basic`. That is how `fprintf` (550 calls) and `vswprintf` got into both ratchets, neither of which the first draft of R8 listed.
...
>
> What is still blind, written down rather than left to be discovered: a printf behind a macro other than `TEXT()`; the comment stripping in both ratchets, which is not string-literal aware; and a *new* printf-shaped method other than `MString::Format`. A finding in any of those falsifies this entry, and the way to look is a sweep over the format argument — never another spelling of the lookup.
>
