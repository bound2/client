...
By severity: **Critical 27 fixed / 1 open** (C19 — briefly marked closed on 2026-09-04 after task 5.4 drove ratchet R7 to 0, and reopened the same day when the review of that slice found ~25 live sites the ratchet cannot see; its entry has them), **High 38 fixed, 3 noted, 19 open** (36/21 until the two build findings closed on 2026-09-02), **Medium 13 fixed / 68 open**, **Low 3 fixed / 25 open**.

**Every finding the review rated Critical is now closed or explicitly narrowed** (C19 is narrowed — see its entry, and note that it was briefly and wrongly marked closed on 2026-09-04). That does not mean the client is free of critical defects: the pointer-truncation pass found a whole family of the same class that the review never recorded, described under *Open, not in this review* below.
...
| Data-file strings as printf formats: 293 call sites checked at the point of use, 24 indirect ones still open (C19, partial) | 🔴 Critical | `31f5f2f`, then `docs/RESTRUCTURING.md` task 5.4 |
...
> ⚠ **Still open.** `docs/RESTRUCTURING.md` task 5.4 converted **294 call sites** across four slices and drove ratchet **R7 to 0**, and on 2026-09-04 this entry was briefly marked Fixed on that basis. **That was wrong, and the adversarial review of the last slice caught it.** R7 counts a call whose format is *spelled at the call site* as a table lookup. It cannot see a table entry that reaches a format **indirectly**, and 24 live sites do exactly that - all of them verified, not estimated: the arrays they read from are initialised entry by entry from the table — the format is read out of a static array or a local the entry was copied into first.
>
> The worst of them is the shape this finding calls the exploitable half, still intact. `VS_UI/src/VS_UI_ExtraDialog.cpp` fills `m_sz_question_msg[][]` from the table in `InitString()`, then does
>
> ```cpp
...
> ```
>
> — a data-file format, **no varargs at all**, into a heap block sized from the format string. A `%s` in that entry reads a stack word as a `char*` and copies it, unbounded, into a buffer sized for something else. 21 sites in that file, 12 of them passing nothing; 3 more in `VS_UI_GameCommon.cpp` (`info_vampire_title_string[num]`, `info_ousters_title_string[num]`, `chingho_name[0]`); and one more was in `Client/PacketHandler/GCSystemMessageHandler.cpp`, which assigned the entry to a local named `pFormat` first — in a directory the first slice reported finished; that one is converted, which is what makes the claim about `Client/PacketHandler` true rather than merely unmeasured.
>
> **What the 293 conversions did buy** is real and unchanged: at every converted site a conversion with no argument left, an argument of the wrong type, a `%n`, or an argument-supplied width is printed as text rather than performed, and every destination is bounded. What is not true is that *no* site remains. Measured: **37 printf-family calls take a format that is not a literal**, of which 24 are data-derived and the rest are vararg forwarders and `TEXT()` literals; **17 pass no varargs**.
>
> Two further limits, unchanged: `tests/tools/check_format_arity.pl` audits arity against the built-in English table — what the default build formats with, since `InitGameStringTable` installs it over the file data — but no `String.inf` ships here, so `LANGUAGE != 3` entries cannot be checked at all and rest on the run-time refusal. And of 386 conversion positions only 117 get a *type* comparison, because an argument whose kind cannot be told is deliberately not a failure.
>
> The four slices and their review rounds are in `docs/RESTRUCTURING.md` task 5.4. What follows is the original narrowing, kept because it is what the numbers above were measured against.
>
...
>
> **All 293 are converted** — every `sprintf`/`wsprintf` in `Client/PacketHandler` fed by the table. `GCBloodBibleListHandler` also `sprintf`ed a `char[192]` plus a `"%3d "` prefix into another `char[192]`, four bytes short, which the bounded formatter closes. Its table lookups are indexed by a packet field and go through `GetGameString()` now — not because the index was unchecked, as the first draft of this note claimed, but because an out-of-range `CTypeTable` lookup yields a default-constructed `MString` whose `GetString()` is NULL, and a NULL reaching a `%s` is undefined rather than empty. The second slice took all 198 `VS_UI` sites the same way, so a third slice took the 27 `AddFormat` sites through a new `CMessageArray::AddSafeFormat`, and a fourth took the last 37 ordinary `sprintf` sites, driving R7 to 0. What R7 counts is now empty; what it cannot count is the ~25 indirect sites above. All three slices are audited by `tests/tools/check_format_arity.pl`, a ctest that compares every converted site's arguments against the built-in English table and fails when an entry asks for more than the call site passes: 286 of the 294 are checkable there, with no failures - the other 8 index the table with a computed id. Neither half of the defence can tell that an entry *means* what its call site passes — a mistranslated entry still prints wrongly. What is gone, at a converted site, is printing something the caller never owned.
